...
const char* const s_importMetaObjectLoadCJS2ESMCode = "(function (z){\"use strict\";var{registry:F}=@Loader,G=@createFIFO(),h=z;while(h){var _=F.@get(h);if((_\?.state\?\?0)<=@ModuleFetch)@fulfillModuleSync(h),_=F.@get(h);var V=@getPromiseInternalField(_.fetch,@promiseFieldReactionsOrResult),w=@Loader.parseModule(h,V),x=_.module;if(w&&@isPromise(w)){var L=@getPromiseInternalField(w,@promiseFieldReactionsOrResult),W=@getPromiseInternalField(w,@promiseFieldFlags),J=W&@promiseStateMask;if(J===@promiseStatePending||L&&@isPromise(L))@throwTypeError(`require() async module \"${h}\" is unsupported. use \"await import()\" instead.`);else if(J===@promiseStateRejected){if(!L\?.message)@throwTypeError(`${L+\"\"\?L:\"An error occurred\"} occurred while parsing module \\\"${h}\\\"`);throw L}_.module=x=L}else if(w&&!x)_.module=x=w;@setStateToMax(_,@ModuleLink);var X=x.dependenciesMap,H=@Loader.requestedModules(x),Q=@newArrayWithSize(H.length);for(var B=0,Y=H.length;B<Y;++B){var D=H[B],T=D[0]===\"/\"\?D:@Loader.resolve(D,h),I=@Loader.ensureRegistered(T);if(I.state<@ModuleLink)G.push(T);@putByValDirect(Q,B,I),X.@set(D,I)}_.dependencies=Q,_.instantiate=@Promise.resolve(_),_.satisfy=@Promise.resolve(_),h=G.shift();while(h&&(F.@get(h)\?.state\?\?@ModuleFetch)>=@ModuleLink)h=G.shift()}var U=@Loader.linkAndEvaluateModule(z,@undefined);if(U&&@isPromise(U))@throwTypeError(`require() async module \\\"${z}\\\" is unsupported. use \"await import()\" instead.`);return @Loader.registry.@get(z)})\n";
...
const int s_importMetaObjectInternalRequireCodeLength = 795;
static const JSC::Intrinsic s_importMetaObjectInternalRequireCodeIntrinsic = JSC::NoIntrinsic;
const char* const s_importMetaObjectInternalRequireCode = "(function (_){\"use strict\";var k=@requireMap.@get(_);const b=_.substring(_.length-5);if(k)return k.exports;if(b===\".json\"){var g=globalThis[Symbol.for(\"_fs\")]||=@Bun.fs(),n=JSON.parse(g.readFileSync(_,\"utf8\"));return @requireMap.@set(_,@createCommonJSModule(_,n,!0)),n}else if(b===\".node\"){const j=@createCommonJSModule(_,{},!0);return process.dlopen(j,_),@requireMap.@set(_,j),j.exports}else if(b===\".toml\"){var g=globalThis[Symbol.for(\"_fs\")]||=@Bun.fs(),n=@Bun.TOML.parse(g.readFileSync(_,\"utf8\"));return @requireMap.@set(_,@createCommonJSModule(_,n,!0)),n}else{var n=@requireESM(_);const v=@requireMap.@get(_);if(v)return v.exports;var q=n\?.default;if(q\?.[@commonJSSymbol]===0)n=q;else n=Object.create(n,{__esModule:{value:!0}});return @requireMap.@set(_,@createCommonJSModule(_,n,!0)),n}})\n";
